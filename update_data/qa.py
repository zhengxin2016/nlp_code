...
class Qa():
    def __init__(self, ip, port, db_name):
        ...
        self.solr_core = 'zx_' + self.db_name + '_qa'
    ...
    def load_data(self):
        qa_path = os.path.join(self.dirpath, 'qa')
        for f in os.listdir(qa_path):
            self.read_data(os.path.join(qa_path, f))

    def write_data2mongodb(self):
        self.db['qa'].drop()
        self.db['qa'].insert(self.qa)
        self.db['qa'].create_index('group')
        self.db['qa'].create_index('label')

    def write_data2solr(self):
        self.solr.delete_solr_core(self.solr_core)
        self.solr.create_solr_core(self.solr_core)
        for x in self.db['qa'].find():
            data_one = x.copy()
